...
Test(thread_local_storage, handle_create) {
  ...
  libd_platform_thread_result_e result = libd_platform_thread_local_storage_handle_create(&handle);
...
Test(thread_local_storage, storage_create) {
  ...
  libd_platform_thread_result_e result = libd_platform_thread_local_storage_create_once(&handle);
  cr_assert(eq(u8, result, RESULT_OK));
