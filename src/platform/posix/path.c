  ...
  enum libd_platform_filesystem_path_type path_type)
{
  ...
  tokenizer_result = libd_filepath_resolver_create(&pt, src_path, path_type);
  ...
  tokenizer_result = libd_filepath_resolver_run(pt);
  if (tokenizer_result != ok) {
    // TODO: make this more coherent
...
cleanup:
  libd_filepath_resolver_destroy(pt);
exit:
  return result;
}

const char*
libd_platform_filesystem_path_string(struct filepath* path)
{
  return path->value;
}
...
_is_valid_portable_path_char(const char c)
{
  return isalnum(c) || c == '.' || c == '-' || c == '_' || c == PATH_SEPARATOR;
}
