                                    ...
                                    uint8_t alignment)
{
  ...
  size_t allocation_size =
    sizeof(linear_allocator_s) + capacity_bytes * sizeof(uint8_t);
  allocation_size = libd_memory_align_offset(allocation_size, alignment);
...
libd_memory_linear_allocator_alloc(linear_allocator_s* p_allocator,
                                   ...
                                   size_t size)
{
  if (p_allocator == NULL || out_param == NULL) {
    return ERR_INVALID_NULL_PARAMETER;
  }
  ...
  *out_param = &p_allocator->dat[p_allocator->bytes_index];
  p_allocator->bytes_index += size;
